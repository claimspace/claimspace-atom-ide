...
// "activationCommands": {
//   "atom-workspace": ["claimspace-atom-ide:toggle", ...]
// },
export default {
  ...
  config: {
    datadir: {
      ...
      type: 'string',
      default: path.join(os.userInfo().homedir, 'Documents', 'Claimspace Projects')
    }
  },
  ...
  activate (state) {
    ...
    this.loginView.onComplete(_ => this.loginModal.hide())
    ...
    this.openProjectView.onAuthError(function () {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'claimspace-atom-ide:toggle':                  () => this.toggle(),
      'claimspace-atom-ide:toggle-native':           () => this.toggleNative(),
      'claimspace-atom-ide:show-open-project-modal': () => this.showOpenProjectModal(),
      'claimspace-atom-ide:show-login-modal':        () => this.showLoginModal()
    }));
    ...
    if (atom.workspace.project.getPaths().length) {
      ...
      if (this.homeView) {
        ...
        atom.workspace.getPanes().forEach(function (pane) {
          console.log(pane.getItems().indexOf(this.homeView))
          pane.destroyItem(this.homeView);
        }.bind(this));
      }
    }
  },
  ...
  possiblyInitializeProjectPath () {
    console.log('possiblyInitializeProjectPath', atom.workspace.alive)
  ...
  initializeProjectPath (root) {
    ...
    this.subnetView = new SubnetView(this.git, subnet);
