...
export default class GitClient {
  ...
  static login (username, password) {
    ...
    return GitProcess.exec(['credential', 'approve'], DefaultPath, { stdin: stdin })
  }

  /**
   *
   */
  static clone (subnetId, location, auth) {
    const remoteUrl = `https://source.claimspace.cloud/${subnetId}.git`;
    ...
    return new Promise(function (resolve, reject) {
      GitProcess.exec([ 'clone', remoteUrl, location ], DefaultPath).then(function (res) {
        if (res.exitCode == 0) {
          ...
          if (res.stderr.indexOf(LoginPattern)) {
            auth();
          } else {
            reject(out(res))
          }
  ...
  getPreviewUrl (subnetId, rev) {
    return this.env.then(function (env) {
      if (env == Env.Local) {
        return `http://staging-${subnetId}-${rev}.local.claimspace.cloud:9000/${subnetId}`;
      } else {
        return `https://staging-${subnetId}-${rev}.claimspace.cloud/${subnetId}`;
      }
    });
  }
  ...
  getLiveUrl (subnetId) {
    return this.env.then(function (env) {
      if (env == Env.Local) {
        return `http://local.claimspace.cloud:9000/${subnetId}`;
      } else {
        return `https://claimspace.cloud/${subnetId}`;
      }
    });
  }
    ...
    return new Promise(function (resolve, reject) {
      git.env.then(function (env) {
        const host = (env == Env.Local) ?
          'http://local.claimspace.cloud:9000' : 'https://claimspace.cloud'
